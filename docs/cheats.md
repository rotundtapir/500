...
| **Reveal hands and the kitty** | Makes the opponents' card piles tappable: tap one to show that hand face up above the felt, tap it again to hide it. **One at a time** — three rows at once crowded the felt off a phone screen, and this way the reveal costs exactly the space already budgeted for the open-misère row (whose renderer it reuses, though not its data path). The kitty turns face up on the felt at the same time: once you can see a hand the kitty is largely inferable anyway, and it costs no extra room since it replaces its own three backs. |
